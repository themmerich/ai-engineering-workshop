...
import {
  ...
  SwitchValueFormatterPipe,
} from '@babs/babs-frontend-shared/lib/formatting/boolean-formatter.pipe';
...
import {
  isBitValue,
  isBooleanValue,
} from '../../../../../../apps/babs-frontend-int/src/app/entities/regel/regel.model';
import { Column } from './table-column.model';

type Row = Record<string, any>;
type NewRow = Record<string, any>;

/** Services/pipes the row formatter needs, supplied by the host component. */
...
/** Safely reads a possibly-nested `a.b.c` field from a row. */
...
/** Status and single-value translation columns. */
function applyTranslatedColumns(
  newRow: NewRow,
  row: Row,
  column: Column,
  ctx: FormatContext,
  ...
  switch (column.type) {
    ...
    case 'platz_status':
      ...
      return true;
    case 'wechsel_art':
      newRow[field] = t.instant(
        'babsappApp.personenprofilWechsel.wechselArt.' + getFieldValue(row, field),
      );
...
/** Array/list columns rendered as chips. */
function applyListColumns(newRow: NewRow, row: Row, column: Column, ctx: FormatContext): boolean {
  ...
  switch (column.type) {
    case 'mitarbeiter_rollen': {
      ...
      newRow[field] = rollen.map((rolle) =>
        t.instant('babsappApp.mitarbeiter.roles.' + rolle.rolle),
      );
      return true;
    }
    case 'news_rollen': {
    ...
    case 'faq_kategorie': {
      ...
      newRow['faqKategoriesValue'] = faqKat.map((kategorie) =>
        t.instant('babsappApp.fAQ.faqKategorien.' + kategorie.kategorie),
      );
    ...
    case 'faq_zielgruppe': {
      ...
      newRow['faqZielgruppesValue'] = faqZiel.map((zielgruppe) =>
        t.instant('babsappApp.fAQ.faqZielgruppen.' + zielgruppe['zielgruppenTyp']),
      );
    ...
    case 'dms_vorlagenart': {
      ...
      newRow[field] = vorlagenart.map((art) =>
        t.instant('babsappApp.dms.maps.vorlagenarten.' + art),
      );
      return true;
    }
    case 'dms_variante': {
      const varianten = row[field] as string[];
      newRow[field] = varianten.map((variante) =>
        t.instant('babsappApp.dms.maps.variante.' + variante),
      );
      return true;
    }
...
/** Composite/link columns (concatenated labels plus the ids the template links to). */
function applyCompositeColumns(newRow: NewRow, row: Row, column: Column): boolean {
  ...
  switch (column.type) {
    ...
    case 'einsatzort_ersteller':
      ...
      newRow['erstelltVonLogin'] =
        `${row['erstelltVonNachname']}, ${row['erstelltVonVorname']} (${row['erstelltVonLogin']})`;
...
/** Person/creator columns (carry the extra name fields the template formats). */
...
/** The `regel_wert` column, whose rendering depends on the rule's key. */
...
/** The `sprache` column, rendered via the language pipes. */
...
const COLUMN_GROUPS = [
  applyTranslatedColumns,
  applyListColumns,
  applyCompositeColumns,
  applyPersonColumns,
  applyRegelWert,
  applySprache,
] as const;

/** Writes the value(s) for a single column into `newRow`, falling back to the raw field value. */
...
/**
 * Transforms the raw rows into display rows: every column value is translated/formatted
 * according to its `type`, and `version`/`id` are carried over for selection/state.
 */
