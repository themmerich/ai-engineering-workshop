...
import {
  faqKategorien,
  faqZielgruppen,
} from '../../../../../../apps/babs-frontend-int/src/app/entities/faq/faq.model';
...
/**
 * Pre-translated option lists used by the column filters (`babs-list-columnfilter`).
 * Each list maps an enum's values through `TranslateService` once, at table init.
 */
...
/** Builds every column-filter option list, translating each enum value via `translate`. */
export function buildFilterOptions(translate: TranslateService): TableFilterOptions {
  const map = (values: string[], prefix: string): string[] =>
    values.map((value) => translate.instant(prefix + value));
  ...
  return {
    ...
    uebersetzungsRichtung: map(
      Object.values(Uebersetzungsrichtung),
      'babsappApp.dolmetscherBuro.uebersetzungsrichtungen.',
    ),
    wiedervorlageGrund: map(Object.values(WiedervorlageGrund), 'babsappApp.wiedervorlage.reason.'),
    wiedervorlageModus: map(
      Object.values(WiedervorlageModus),
      'babsappApp.wiedervorlage.wvView.modus.',
    ),
    wiedervorlageStand: map(
      Object.values(WiedervorlageStand),
      'babsappApp.wiedervorlage.wvView.stand.',
    ),
