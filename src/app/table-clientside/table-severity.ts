...
/** PrimeNG tag severities used by the table's status columns. */
export type Severity = 'success' | 'secondary' | 'info' | 'warn' | 'danger' | 'contrast' | null;

/**
 * Returns the severity of the first entry whose translated key equals `value`,
 * or `null` if none match — mirroring the original ordered if/else chains.
 */
function firstSeverity(
  translate: TranslateService,
  value: string,
  entries: readonly [string, Exclude<Severity, null>][],
): Severity {
...
/** Severity for the Dolmetscher status column. */
